...
export class MovieService {
  ...
  public createMovie(newMovie: Movie) {
      this.httpClient.post(this.serverUrl, JSON.stringify(newMovie), 
      {headers:{'Content-Type': 'application/json'}, responseType:'text'})
      .pipe(
        concatMap(v => this.httpClient.get<Array<Movie>>(this.serverUrl + 'addMovie')),
        catchError(err => {
